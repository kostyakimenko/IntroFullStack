const ATM = {
    ...
    auth: function(number, pin) {
        if (this.is_auth) {
            return this.logging(this.message.reportQueueErr, this.message.queueErr, false);
        }

        if (typeof number !== 'string' || typeof pin !== 'string') {
            return this.logging(this.message.reportParamErr, this.message.paramErr, false);
        }
        ...
        return this.logging(this.message.reportUserNotFound, this.message.userNotFound, false);
    },

    /* Check current debet */
    check: function() {
        if (!this.is_auth) {
            return this.logging(this.message.reportUnauthorized, this.message.unauthorized, false);
        }
    ...
    getCash: function(amount) {
        ...
        if (amount > this.current_user.debet) {
            return this.logging(this.message.reportUserCashDeficit, this.message.userCashDeficit, false);
        }

        if (amount > this.cash) {
            return this.logging(this.message.reportAtmCashDeficit, this.message.atmCashDeficit, false);
        }

        this.current_user.debet -= amount;
        this.cash -= amount;

        return this.logging(`user ${this.current_user.number} get ${amount}`, `You get ${amount}`, true);
    },
    ...
    logout: function() {
        ...
        this.logging(`${this.current_type} ${this.current_user.number} is logout`, this.message.logOut, true);
